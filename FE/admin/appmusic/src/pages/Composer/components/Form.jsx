...
import { composerSchema } from '../../../utils/validate.form'
...
const Form = ({ composer, onSubmit, isLoading }) => {
  // react-hook-form
  const {
    reset,
    control,
  ...
  useLayoutEffect(() => {
    ...
    const initialFormValue = async (composer) => {
      if (composer !== undefined) {
        const filesDownload = await Promise.all([
          downloadFile(composer.avatarUrl, 'image/jpeg'),
          downloadFile(composer.backgroundImageUrl, 'image/jpeg')
        ])
        const artistInitial = {
    ...
    initialFormValue(composer)
  }, [composer, setValue, reset])

  // submit form
  const handleSubmitForm = handleSubmit(async (data) => {
    onSubmit(composer, data, setError)
  })
