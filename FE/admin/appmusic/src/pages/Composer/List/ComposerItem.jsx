...
const ComposerItem = ({ composer }) => {
  ...
  const deleteComposerMutation = useMutation({
    mutationFn: (id) => composerApi.deleteComposer(id)
  })
    ...
    <>
      <div className='col-span-4 h-72 w-full rounded-3xl bg-white shadow'>
        ...
        <div className='text-center'>
          <span className='text-lg font-semibold text-gray-600'>{composer.fullName}</span>
        </div>
        <div className='text-center'>
          <span className='text-base font-medium text-gray-400'>{`${composer.songs.length} songs`}</span>
        </div>
      </div>
      {isShowDeleteModal && (
        <Modal onClose={() => setIsShowDeleteModal(false)}>
          ...
          >
            ...
            <div className='flex h-full items-center justify-center'>
              <h3 className='text-center text-xl font-medium'>{`Are you sure to delete the composer "${composer.fullName}"?`}</h3>
            </div>
            <div className='flex items-center justify-center gap-10'>
              <button
                onClick={async () => {
                  await deleteComposerMutation.mutateAsync(composer.id, {
                    onSuccess: () => {
                      queryClient.invalidateQueries(['composers', { ...queryParams.fullName }])
                    }
                  })
                  setIsShowDeleteModal(false)
                }}
                type='button'
                disabled={deleteComposerMutation.isLoading}
