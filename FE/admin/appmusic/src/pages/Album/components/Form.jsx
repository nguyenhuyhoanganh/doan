...
import { albumSchema } from '../../../utils/validate.form'
import Input from '../../../components/Input'
import RichText from '../../../components/RichText/RichText'
import UploadImage from '../../../components/UploadImage/UploadImage'

const Form = ({ album, onSubmit, isLoading }) => {
  // react-hook-form
  const {
    reset,
    control,
  ...
  useLayoutEffect(() => {
    ...
    const initialFormValue = async (album) => {
      if (album !== undefined) {
        const filesDownload = await Promise.all([downloadFile(album.backgroundImageUrl, 'image/jpeg')])
        const albumInitial = {
          title: album.title,
          description: album.description,
          backgroundImage: filesDownload[0]
        }
        reset(albumInitial)
      }
    }
    initialFormValue(album)
  }, [album, setValue, reset])

  // submit form
  const handleSubmitForm = handleSubmit(async (data) => {
    onSubmit(album, data, setError)
  })
