...
import { categorySchema } from '../../../utils/validate.form'
import Input from '../../../components/Input'
import RichText from '../../../components/RichText/RichText'

const Form = ({ category, onSubmit, isLoading }) => {
  // react-hook-form
  const {
    reset,
    control,
  ...
  useLayoutEffect(() => {
    const initialFormValue = async (category) => {
      if (category !== undefined) {
        const categoryInitial = {
          title: category.title,
          description: category.description
        }
        reset(categoryInitial)
      }
    }
    initialFormValue(category)
  }, [category, setValue, reset])

  // submit form
  const handleSubmitForm = handleSubmit(async (data) => {
    onSubmit(category, data, setError)
  })

  return (
    <form onSubmit={handleSubmitForm} noValidate>
      <div className='mt-7 grid grid-cols-12 gap-6 rounded-lg'>
