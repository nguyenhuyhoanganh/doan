import React, { useState } from 'react'
import useQueryParams from '../../../hoocs/useQueryParams'
import { useMutation, useQueryClient } from '@tanstack/react-query'
import categoryApi from '../../../apis/category.api'
import Modal from '../../../components/Modal/Modal'
import { MdOutlineClose } from 'react-icons/md'
import CategoryEdit from '../Edit'

const CategoryItem = ({ category, queryConfig }) => {
  const [isShowDeleteModal, setIsShowDeleteModal] = useState(false)
  const [isShowEditModal, setIsShowEditModal] = useState(false)
  const queryParams = useQueryParams()
  const queryClient = useQueryClient()
  // mutation
  const deleteCategoryMutation = useMutation({
    mutationFn: (id) => categoryApi.deleteCategory(id)
  })

  return (
    <tr className='max-h-14 bg-white'>
      <td className={`block w-40 items-center px-6 py-3 text-left font-medium`}>
        <span className={`m-0 min-h-[1.5rem] line-clamp-2`}>{category.title}</span>
      </td>
      <td className={`w-full items-center px-6 py-3 text-left font-medium`}>
        <span className={`m-0 min-h-[1.5rem] line-clamp-2`}>{category.description}</span>
      </td>
      <td className={`flex min-w-[6rem] items-center justify-center gap-3 px-6 py-3 text-left font-medium`}>
        <span onClick={() => setIsShowEditModal(true)}>
          <svg
            xmlns='http://www.w3.org/2000/svg'
            fill='none'
            viewBox='0 0 24 24'
            strokeWidth={1.5}
            stroke='currentColor'
            className='h-6 w-6'
          >
            <path
              strokeLinecap='round'
              strokeLinejoin='round'
              d='M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10'
            />
          </svg>
        </span>
        <span className='text-red-500' onClick={() => setIsShowDeleteModal(true)}>
          <svg
            xmlns='http://www.w3.org/2000/svg'
            fill='none'
            viewBox='0 0 24 24'
            strokeWidth={1.5}
            stroke='currentColor'
            className='h-6 w-6'
          >
            <path
              strokeLinecap='round'
              strokeLinejoin='round'
              d='M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0'
            />
          </svg>
        </span>
      </td>
      {isShowDeleteModal && (
        <Modal onClose={() => setIsShowDeleteModal(false)}>
          <div
            onClick={(e) => e.stopPropagation()}
            className={`relative flex h-60 w-[27rem] flex-col rounded-lg bg-white px-4 pt-10 pb-8`}
          >
            <span
              onClick={() => setIsShowDeleteModal(false)}
              className='absolute right-3 top-3 inline-flex cursor-pointer items-center justify-center font-[30px] text-red-400'
            >
              <MdOutlineClose size={32} />
            </span>
            <div className='flex h-full items-center justify-center'>
              <h3 className='text-center text-xl font-medium'>{`Are you sure to delete the category "${category.title}"?`}</h3>
            </div>
            <div className='flex items-center justify-center gap-10'>
              <button
                onClick={async () => {
                  await deleteCategoryMutation.mutateAsync(category.id, {
                    onSuccess: () => {
                      queryClient.invalidateQueries(['categories', { ...queryParams.title }])
                    }
                  })
                  setIsShowDeleteModal(false)
                }}
                type='button'
                disabled={deleteCategoryMutation.isLoading}
                className='mr-2 mb-2 block rounded-lg bg-green-400 px-5 py-2.5 text-sm font-medium text-white hover:bg-green-500'
              >
                Delete
              </button>
              <button
                onClick={() => setIsShowDeleteModal(false)}
                type='button'
                className='mr-2 mb-2 block rounded-lg bg-red-400 px-5 py-2.5 text-sm font-medium text-white hover:bg-red-500'
              >
                Cancel
              </button>
            </div>
          </div>
        </Modal>
      )}
      {isShowEditModal && (
        <Modal onClose={() => setIsShowEditModal(false)}>
          <div
            onClick={(e) => e.stopPropagation()}
            className={`relative flex h-[35rem] w-[60rem] flex-col rounded-lg bg-white px-4 pt-10 pb-8`}
          >
            <span
              onClick={() => setIsShowEditModal(false)}
              className='absolute right-3 top-3 inline-flex cursor-pointer items-center justify-center font-[30px] text-red-400'
            >
              <MdOutlineClose size={32} />
            </span>
            <CategoryEdit onClose={() => setIsShowEditModal(false)} cateId={category.id} />
          </div>
        </Modal>
      )}
    </tr>
  )
}

export default CategoryItem
